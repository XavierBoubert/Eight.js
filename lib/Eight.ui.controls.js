			...
			for(var i = 0; i < _controls.length; i++) {
				if(_controls[i].type == type) {
					for(var j = 0; j < _controls[i].items.length; j++) {
						if(!_isEmpty(_controls[i].items[j].el) && _controls[i].items[j].el.DOM.id == id) {
...
(function(window) {
	...
	$8.Templates.define('control_category_container',
		'<div id="{{ id }}_container" class="{{ baseClass }}-container">' +
			'<h2 id="{{ id }}_title" class="{{ baseClass }}-title"><a href="{{ link }}">{{ title }}</a></h2>' +
			'<div id="{{ id }}_content" class="{{ baseClass }}-content">' +
				'{{ content }}' +
			'</div>' +
		'</div>'
	...
	$8.UI.Controls.register(
		...
		function constructor(el, params) {
			...
			el.addClass('h8-control-category');
			if(params.cls !== '') {
				el.addClass(params.cls);
			}

			var baseClass = 'h8-control-category',
				id = el.DOM.id,
				content = el.html(),
				html = '';
			...
			$8.UI.Controls.setParams('category', id, params);
...
(function(window) {
	...
	$8.Templates.define([[
		...
		'<a id="{{ id }}_container" class="{{ baseClass }}-container" href="{{ link }}">' +
		...
		'<a id="{{ id }}_container" class="{{ baseClass }}-container" href="{{ link }}">' +
			'<div class="{{ baseClass }}-nopicture-content">' +
				'<div class="{{ baseClass }}-nopicture-text">' +
					'{{ text }}' +
				'</div>' +
			'</div>' +
		'</a>'
	...
	$8.UI.Controls.register(
		...
		function constructor(el, params) {
			...
			$8.UI.Controls.setParams('tile', id, params);
