		...
		function createControl(type, el, params) {
			...
			for(var i = 0; i < _controls.length; i++) {
				if(_controls[i].type == type) {
					...
					return _controls[i].constructor(el, params);
				}
			}
		}
...
(function(window) {
	...
	$8.UI.Controls.register('scroller',
		...
		function constructor(el, params) {
			var scroller = $8.UI.Controls.scroller,
				id = el.DOM.id,
				html = '',
				content = el.html(),
...
(function(window) {
	...
	$8.Templates.define([[
		...
		'<a id="{{ id }}_container" class="{{ baseCls }}-container" href="{{ link }}">' +
			'<img id="{{ id }}_picture" class="{{ baseCls }}-picture" src="{{ picture }}" />' +
	...
	$8.UI.Controls.register('tile',
		...
		function constructor(el, params) {
			...
			if(params.picture !== '') {
				...
				picEl.DOM.addEventListener('load', $8.UI.Controls.tile.updatePicture);

				if(params.pictureHover !== '') {
					containerEl.DOM.addEventListener('mouseover', function() {
						picEl.DOM.src = params.pictureHover;
					});
					containerEl.DOM.addEventListener('mouseout', function() {
		...
		// Methods
		{
			updatePicture: function(e) {
				var id = e.target.id.replace('_picture', ''),
					picEl = $8.UI.el(e.target),
					params = $8.UI.Controls.getParams(id);
				...
				picEl.DOM.removeEventListener('load', $8.UI.Controls.tile.updatePicture);
			},
		...
		function constructor(el, params) {
			...
			function lostCapture() {
				...
				setTimeout(function() {
					if(!capture && opened) {
						opened = false;
						$8.UI.Controls.appbar.hide(el);
					}
				}, 1000);
			}

			el.DOM.addEventListener('mouseover', function(e) {
				capture = true;
				if(!opened) {
					opened = true;
					$8.UI.Controls.appbar.show(el);
				}
			});
			el.DOM.addEventListener('mouseout', function(e) {
				lostCapture();
			});
			containerEl.DOM.addEventListener('mouseover', function(e) {
				capture = true;
			});
		},
		// Methods
...
(function(window) {
	...
	$8.UI.Controls.register('command',
		...
		function constructor(el, params) {
			...
			el.DOM.addEventListener('click', function(e) {
				if(params.action) {
					params.action(e);
				}
			});
	...
	$8.UI.Controls.register('parameters',
		...
		function constructor(el, params) {
			...
			window.addEventListener('resize', function(e) {
				$8.UI.Controls.parameters.refresh(el);
			});
			...
			function lostCapture() {
				...
				setTimeout(function() {
					if(!capture && opened) {
						opened = false;
						$8.UI.Controls.parameters.hide(el);
					}
				}, 1000);
			}
			...
			el.DOM.addEventListener('mouseover', function(e) {
				...
				if(!opened) {
					opened = true;
					$8.UI.Controls.parameters.show(el);
				}
			});
			el.DOM.addEventListener('mouseout', function(e) {
				lostCapture();
