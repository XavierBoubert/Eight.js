	...
	$8.UI.Controls.register('scroller',
		...
		function constructor(scroller, el, params) {
			...
			var elContent = $8.UI.el(id + '_content'),
				...
				scrollsVisibility = false,
				scrollsForceVisibility = false;
			...
			if(params.hiddenScrolls) {
				...
				$8.Utilities.addEvent(el.DOM, 'mouseout', function() {
					scrollsVisibility = false;
					setTimeout(function() {
						if(!scrollsVisibility && !scrollsForceVisibility) {
		...
		// Methods
		{
			refresh: function(el) {
				...
				if((params.orientation == 'vertical' || params.orientation == 'both') && elHeight < elContentHeight) {
				...
				if((params.orientation == 'horizontal' || params.orientation == 'both') && elWidth < elContentWidth) {
			...
			position: function(el, orientation, position) {
				var id = el.DOM.id,
					params = $8.UI.Controls.getParams(id),
					elHorizontal = $8.UI.el(id + '_horizontal'),
					elVertical = $8.UI.el(id + '_vertical'),
				...
				if(orientation == 'vertical' && elVertical.style('display') == 'block') {
				...
				else if(orientation == 'horizontal' && elHorizontal.style('display') == 'block') {
			...
			scrollBy: function(el, increment) {
				...
				if((params.orientation == 'vertical' || params.orientation == 'both') && elVertical.style('display') == 'block') {
					var elVerticalTicker = $8.UI.el(id + '_vertical_ticker');
					var top = elVerticalTicker.style('top') || 0;
					top = top !== 0 ? parseInt(top.replace('px', ''), 10) : top;
					top += increment;

					$8.UI.Controls.scroller.position(el, 'vertical', top);
				}
				else if((params.orientation == 'horizontal' || params.orientation == 'both') && elHorizontal.style('display') == 'block') {
					var elHorizontalTicker = $8.UI.el(id + '_horizontal_ticker');
					var left = elHorizontalTicker.style('left') || 0;
					left = left !== 0 ? parseInt(left.replace('px', ''), 10) : left;
					left += increment;

					$8.UI.Controls.scroller.position(el, 'horizontal', left);
				}
			},
