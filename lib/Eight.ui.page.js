(function(window) {
	...
	$8.Templates.define([[
		...
		'<div id="{{ id_title }}" class="h8-title">' +
			'<h1></h1>' +
		'</div>' +
		'<div id="{{ id_content }}" class="h8-page-content" data-control="scroller" data-params="{orientation: \'{{ content_orientation }}\'}" style="width: {{ content_width }}px; height: {{ content_height }}px;">' +
			'{{ content }}' +
		'</div>'
	],[
	...
	var Page = function() {
		...
		this.define = function(name, options) {
			...
			if(!$8.Utilities.isDOMReady) {
				...
				if(!_inArray(options.layout, _layoutTypes)) {
					options.layout = 'hub';
				}
				...
				var content = _layoutFunctions[options.layout](options);

				_orientation = content.orientation;

				if(_orientation == 'horizontal') {
					content.width = screenSize.width - (6 * $8.UI.Unit.width);
					content.height = screenSize.height - (9.5 * $8.UI.Unit.height);
				}
				else {
					content.width = screenSize.width - (6 * $8.UI.Unit.width);
					content.height = screenSize.height - (7 * $8.UI.Unit.height);
				}

				html += $8.Templates.h8_page_container({
					id_title: idTitle,
					id_content: idContent,
					content_orientation: content.orientation,
		...
		function makeLayout(content, options) {
			...
			if(options.items.length === 0) {
				...
				content.html = options.items.join('');
			}

			return content;
		}

		function layoutHub(options) {
			var content = {
				orientation: 'horizontal', // force
				html: ''
			};

			return makeLayout(content, options);
		}

		function layoutSection(options) {
			var content = {
				orientation: 'horizontal', // force
				html: 'section'
			};
		...
		this.refresh = function() {
			...
			_els.content.style('width', width + 'px')
						.style('height', height + 'px');
		};
